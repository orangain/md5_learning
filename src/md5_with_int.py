...
def md5_hexdigest(string):
    input_bytes = string.encode('utf8')
    # ステップ1. メッセージの拡張
    padded_bytes = add_padding(input_bytes)
    assert (len(padded_bytes) * 8) % 512 == 448, padded_bytes

    # ステップ2. メッセージに長さ付加
    padded_bytes_with_length = add_length(input_bytes, padded_bytes)
    assert len(padded_bytes_with_length) * 8 % 512 == 0
    ...
    # ステップ4. 算出処理
    for i in range(0, len(padded_bytes_with_length), 64):
        target_bytes = padded_bytes_with_length[i:i+64]
        target_words = bytes_to_words(target_bytes)
        A, B, C, D = calculate_MD5(target_words, A, B, C, D)
...
def add_padding(input_bytes):
    ...
    if num_bits % 512 >= 448:
        num_padding_bits = 448 + 512 - num_bits % 512
    else:
        num_padding_bits = 448 - num_bits % 512
...
def add_length(input_bytes, padded_bytes):
    num_bits = len(input_bytes) * 8
    return padded_bytes + int_to_bytes(num_bits, 8)
...
for i in range(64):
    T.append(WORD(int(4294967296 * abs(sin(i + 1)))))
